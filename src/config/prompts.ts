...
const DEFAULT_PLANNER_SYSTEM = `You are an expert software engineer answering questions about THIS codebase. 
...
const DEFAULT_STEP_SYSTEM = `You are solving ONE step from the plan. 
...
const DEFAULT_STEP_DEVELOPER = `Context: You are analyzing a Guidewire Gosu codebase indexed by gosu-rag into semantic chunks.
...
const DEFAULT_EVALUATOR_SYSTEM = `Compare the recent StepOutcome to the remaining plan steps.
...
const DEFAULT_FINALIZER_SYSTEM = `Produce the final answer to the user's question, grounded in the retrieved code from all completed steps.
...
/**
 * Load prompt content from file, falling back to default if file likely missing
 */
function loadPrompt(promptPath: string, defaultContent: string): string {
    try {
        ...
        console.warn(`[Config] Failed to load prompt from ${promptPath}, using default.`);
    }
...
export function getPlannerSystemPrompt(): string {
    ...
    return loadPrompt(config.promptPlannerSystemPath, DEFAULT_PLANNER_SYSTEM);
}

export function getStepSystemPrompt(): string {
    const config = loadConfig();
    return loadPrompt(config.promptStepSystemPath, DEFAULT_STEP_SYSTEM);
}

export function getStepDeveloperPrompt(): string {
    const config = loadConfig();
    return loadPrompt(config.promptStepDeveloperPath, DEFAULT_STEP_DEVELOPER);
}

export function getEvaluatorSystemPrompt(): string {
    const config = loadConfig();
    return loadPrompt(config.promptEvaluatorSystemPath, DEFAULT_EVALUATOR_SYSTEM);
}

export function getFinalizerSystemPrompt(): string {
    const config = loadConfig();
    return loadPrompt(config.promptFinalizerSystemPath, DEFAULT_FINALIZER_SYSTEM);
}
